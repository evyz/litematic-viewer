...
export default function BlockListButton({ modalType, setModalType }: Props) {
    return (
        <Button variant={"outline"} className={"size-10 rounded-full"} onClick={() => setModalType(prev => prev !== null ? null : 'block_list')}>
            {modalType === "block_list" ? <ActiveIcon /> : <Icon />}
        </Button>
    )
}
